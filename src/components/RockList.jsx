    ...
    const displayRocks = () => {
        if (rocks && rocks.length) {
            return rocks.map(rock => 
                ...
                <div>{rock.name} ({rock.type.label}) weights {rock.weight}</div>
                    ...
                    <button
                        onClick={async () => {
                            const response = await fetch(`http://localhost:8000/rocks/${rock.id}`,
                                ...
                                if (response.status === 204) {fetchRocks(showAll)
                            }
                        }}
                        className="border border-solid bg-red-700 text-white p-1">Delete
                    </button>
                </div>  
